...
void TreeStandardizer::WITHIN(treeNode* root){
    ...
    e1Node = x1Node->sibling;           // e2Node=x2Node->sibling
...
void TreeStandardizer::REC(treeNode* root){
    ...
    xNode->sibling = NULL;
    root->child = xNode; //removing the reference 
    gammaNode = new treeNode();   // create a new node
...
void TreeStandardizer::FCNFORM(treeNode* root){
    ...
    root->type = treeNode::BINDING;
    root->nodeString = "=";
    vNode = root->child;     // set the P node
